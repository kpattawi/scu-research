...
public class Controller extends ControllerBase {
    ...
    String[] dataStrings = new String[]{"","","","","","","","",""}; // I think this needs to be initiallized... can avoid in future if I change the name for the send controls part dataStrings
    //  Didnt need to chanGE LINE above... I just forgot to change numSockets to 9
    ...
    private void execute() throws Exception {
        ...
        while (!exitCondition) {
          ...
          for(int i=0;i<numSockets;i++){
            ...
            // // For NIST PRESENTATION------------------------
            // heatTemps[i] = zoneTemps[i] - Double.valueOf(i)*0.5 -0.5;
            // coolTemps[i] = zoneTemps[i] + Double.valueOf(i)*0.5 +0.5;
            // -----------------------------------------
            if (i==0){
              heatTemps[i] = heatTemps[i]+1;
              coolTemps[i] = coolTemps[i]+1;
            }else if (i==1){
              heatTemps[i] = heatTemps[i]-1;
              coolTemps[i] = coolTemps[i]-1;
            }else if (i==3){
