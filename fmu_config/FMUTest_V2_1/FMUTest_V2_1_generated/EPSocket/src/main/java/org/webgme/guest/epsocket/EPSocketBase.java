...
public class EPSocketBase extends SynchronizedFederate {
    private SubscribedInteractionFilter _subscribedInteractionFilter =
        new SubscribedInteractionFilter();

    // constructor
    public EPSocketBase(FederateConfig config) throws Exception {
        super(config);
        super.createLRC();
        ...
        // interaction pubsub
        ReceiveFromEP.publish(getLRC());
        SendToEP.subscribe(getLRC());
    ...
    public ReceiveFromEP create_ReceiveFromEP() {
        ReceiveFromEP interaction = new ReceiveFromEP();
